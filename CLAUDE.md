...
| `market_matcher.py` | Pairs each `NormalizedMarket` to a Pinnacle line (2-way or 3-way). Team markets anchor via ML sibling keyed on `adapter.event_group_key`; props resolve by `(matchupId, canonical_stat, player_key)`. Fails closed on unknown league. | Match logic, period map, prop index, 3-way ML handling (see Conventions), `unknown_league` drops. |
...
| `find_ev_bet.py` | One-shot +EV scanner. 2-way multiplicative devig on `[yes_price, opposite_price]` (3-way soccer ML pre-flattened in the matcher). Per-book ladder walk under that book's `fee_fn`. Drops started games; tags `in_window` for 0.5–3h. | EV math, devig method, start-time window. |
...
| `paper_tracker.py` | Simulates one bet per `{book}:{market_id}:{side}` on entry to 0.5–3h window. Fractional Kelly (f=0.25) off $5000 compounding bankroll. Per-Pinnacle-matchup stake cap `PER_MATCH_STAKE_CAP_PCT=0.03` bounds correlated exposure (all bets sharing a `pin_matchup_id` ≤ 3% of bankroll). `PER_MATCH_BET_CAP=2` additionally caps open-bet count per matchup to limit variance from correlated legs. Per-book price-aware gate: `max(MIN_EDGE_PCT, 100 × fee_rate × (1-px) + margin_pp)` — `PER_BOOK_FEE_RATE` / `PER_BOOK_EDGE_MARGIN_PP` drive it. YES/NO independent. Closes captured in side-perspective. → `data/paper_{trades,settlements,closes}.jsonl`. | Bankroll rules, Kelly fraction, per-match cap, edge gate, CLV capture, net_ev semantics, side-aware dedupe. |
...
- Per-matchup caps in paper-tracker: `PER_MATCH_STAKE_CAP_PCT=0.03` bounds correlated stake (≤3% of bankroll across all bets sharing `pin_matchup_id`); `PER_MATCH_BET_CAP=2` caps open-bet count per matchup to limit variance from correlated legs. Both gates skip when `pin_matchup_id` is None. Note: applies to 3-way soccer ML too — third leg dropped if first two clear edge gate.
- Team-market scope: ML / spread / total / team-total; half-point lines; FULL + 1H/2H. Polymarket US is FULL-game ML/spread/total only.
- 3-way soccer ML (Kalshi-only): one `MatchedPair` per outcome. `yes_designation ∈ {home, away, draw}`; `opposite_side_price` synthesized via `synthesize_combined_american` of the other two Pinnacle legs (max ~0.03pp drift vs full 3-way devig). `opposite_designation = "not_{home,away,draw}"` is a sentinel — paper-tracker's `_find_pin_prices` re-synthesizes on that prefix at CLV capture. Polymarket draw support pending a live soccer snapshot.
...
- Close capture: team markets one-shot in `[start − 60s, start + 15m]`; player props capture continuously from placement (last-seen fair prob wins) since Pinnacle drops props at game start. JSONL writes throttled on unchanged `fair_prob_close`; replay is last-write-wins.
